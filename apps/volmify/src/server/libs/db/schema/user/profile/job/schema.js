...
import { numericCols, table, temporalCols, textCols } from "../../../_utils/helpers";
...
export const userJobProfile = table(
	...
	(t) => [
		...
		uniqueIndex(`uq_${userJobProfileTableName}_slug`).on(t.slug),
		index(`idx_${userJobProfileTableName}_user_profile_id`).on(t.userProfileId),
		index(`idx_${userJobProfileTableName}_verified_at`).on(t.verifiedAt),
		index(`idx_${userJobProfileTableName}_created_at`).on(t.createdAt),
		index(`idx_${userJobProfileTableName}_last_updated_at`).on(t.lastUpdatedAt),
		// check("job_profile_type_enforcement",
...
// const userJobProfileI18nTableName = `${userTableName}_job_profile_i18n`;

...
import type { ActivityTracker, ResearchFindings, ResearchState, SearchResult } from "./types";
...
export async function extractContent(
	...
	try {
		...
		return handleError(error, `Content extraction from ${url}`, activityTracker, "extract", null);
	}
}
...
export async function processSearchResults(
	...
	for (const item of extractionResults) {
		if (item.status === "fulfilled" && item.value !== null && item.value !== undefined) {
					...
					queries: z
						.array(
							z.object({
								...
								purpose: z.string().describe("The purpose of this search query"),
								priority: z
									.enum(["high", "medium", "low"])
									.describe("Priority of the search query"),
								expectedInsight: z.string().describe("Expected insight from this search query"),
							}),
						)

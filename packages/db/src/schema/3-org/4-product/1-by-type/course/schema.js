...
export const orgProductCourseLevelEnum = tEnum(`${orgProductCourseTableName}_level`, [
	"beginner",
	"intermediate",
	"advanced",
	"expert",
...
export const orgProductCourseSkill = table(
	...
	(cols) => [
		...
		check({
			tName: orgProductCourseSkillTableName,
			postfix: "weight_range",
			condition: sql`${cols.weight} >= 1 AND ${cols.weight} <= 10`,
		}),
	],
);
...
export const orgMemberProductCourseChallengeRatingHistoryChangeReasonEnum = tEnum(
...
export const orgProductCourseModule = table(
	...
	(cols) => [
		...
		check({
			tName: orgProductCourseModuleTableName,
			postfix: "required_access_tier_range",
			condition: sql`${cols.requiredAccessTier} >= 0`,
		}),
		check({
			tName: orgProductCourseModuleTableName,
			postfix: "sort_order_range",
			condition: sql`${cols.sortOrder} >= 0`,
		}),
		...multiIndexes({
			tName: orgProductCourseModuleTableName,
			colsGrps: [
...
export const orgMemberProductCourseEnrollmentStatusEnum = tEnum(

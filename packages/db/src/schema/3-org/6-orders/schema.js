...
import { numericCols } from "../../_utils/cols/numeric";
import { temporalCols } from "../../_utils/cols/temporal";
import { textCols } from "../../_utils/cols/text";
import { check, multiForeignKeys, multiIndexes, uniqueIndex } from "../../_utils/helpers";
import { table, tEnum } from "../../_utils/tables.js";
import { currencyCodeFkCol } from "../../0-local/0_utils";
import { orgTableName } from "../_utils/index";
import { orgIdFkCol, orgIdFkExtraConfig } from "../0_utils";
import { orgMemberIdFkCol, orgMemberIdFkExtraConfig } from "../1-member-and-employee/0_utils";
import { orgTaxRateSnapshot } from "../3-tax/schema";
import { orgProduct, orgProductVariant } from "../4-product/00-schema";
import { orgProductVariantPaymentPlan } from "../4-product/payment/schema";
import { orgCoupon, orgDiscount, orgGiftCard } from "../5-offers/schema";

// ## org -> product -> order
const orgMemberOrderTableName = `${orgTableName}_member_order`;

// ✅ ORDER STATUS: Complete e-commerce lifecycle
export const orgMemberOrderStatusEnum = tEnum(`${orgMemberOrderTableName}_status`, [
...
export const orgMemberOrderPaymentStatusEnum = tEnum(`${orgMemberOrderTableName}_payment_status`, [
...
export const orgMemberOrder = table(
	...
	(cols) => [
		...
		check({
			tName: orgMemberOrderTableName,
			postfix: "total_amount_calculation",
			condition: sql`${cols.totalAmount} = ${cols.subtotalAmount} - ${cols.totalDiscountAmount} + ${cols.totalTaxAmount}`,
		}),
		check({
			tName: orgMemberOrderTableName,
			postfix: "positive_amounts",
			condition: sql`${cols.subtotalAmount} >= 0 AND ${cols.totalDiscountAmount} >= 0 AND ${cols.totalTaxAmount} >= 0`,
		}),
		check({
			tName: orgMemberOrderTableName,
			postfix: "valid_payment_timing",
			condition: sql`${cols.paidAt} IS NULL OR ${cols.paidAt} >= ${cols.orderedAt}`,
		}),
		check({
			tName: orgMemberOrderTableName,
			postfix: "valid_fulfillment_timing",
			condition: sql`${cols.fulfilledAt} IS NULL OR ${cols.fulfilledAt} >= ${cols.orderedAt}`,
		}),
	],
);
...
export const orgMemberOrderItem = table(
	...
	(t) => [
		...
		check({
			tName: orgMemberOrderItemTableName,
			postfix: "positive_quantity",
			condition: sql`${t.quantity} > 0`,
		}),
		check({
			tName: orgMemberOrderItemTableName,
			postfix: "total_price_calculation",
			condition: sql`${t.totalPrice} = ${t.unitPrice} * ${t.quantity}`,
		}),
		check({
			tName: orgMemberOrderItemTableName,
			postfix: "valid_access_tier",
			condition: sql`${t.selectedAccessTier} >= 0`,
		}),
	],
);
...
export const orgMemberOrderDiscount = table(
	...
	(t) => [
		...
		check({
			tName: orgMemberOrderDiscountTableName,
			postfix: "positive_discount",
			condition: sql`${t.discountAmount} >= 0`,
		}),
		check({
			tName: orgMemberOrderDiscountTableName,
			postfix: "single_discount_source",
			condition: sql`(${t.discountId} IS NOT NULL)::int + (${t.couponId} IS NOT NULL)::int + (${t.giftCardId} IS NOT NULL)::int = 1`,
		}),
	],
);
...
export const taxCalculationMethodEnum = tEnum(`${orgMemberOrderTableName}_tax_calculation_method`, [
...
export const paymentGatewayEnum = tEnum("payment_gateway", [
	// "stripe",
	// "paypal",
	// "square",
	// "braintree",
...
export const paymentMethodEnum = tEnum("payment_method", [
	"credit_card",
	"debit_card",
	"paypal",
	"apple_pay",
...
// export const paymentTransactionStatusEnum = tEnum("payment_transaction_status", [
// 	"pending",
// 	"authorized",
// 	"captured",
// 	"settled",
...
export const orgMemberOrderPayment = table(
	...
	(t) => [
		...
		check({
			tName: orgMemberOrderPaymentTableName,
			postfix: "amount_calculation",
			condition: sql`${t.netAmount} = ${t.grossAmount} - ${t.processingFee}`,
		}),
		check({
			tName: orgMemberOrderPaymentTableName,
			postfix: "positive_amounts",
			condition: sql`${t.grossAmount} >= 0 AND ${t.processingFee} >= 0`,
		}),
		...multiIndexes({
			tName: orgMemberOrderPaymentTableName,
			colsGrps: [

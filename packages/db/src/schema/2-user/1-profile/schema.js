...
export const userProfileTableName = `${userTableName}_profile`;
export const userProfileTypeEnum = tEnum("user_profile_type", [
...
export const { extraConfig: userProfileIdFkExtraConfig, fkCol: userProfileIdFkCol } = buildFkUtils({
	...
	table: userProfile,
	// getTable: async () => (await import("#schema/user/profile/job/schema.js")).userProfile,
...
// 		periodType: tEnum("period_type", ["daily", "weekly", "monthly"])("period_type"),
...
export const userJobProfileVerification = table(
  'user_job_profile_verification',
  {
    ...
    // Verification types
    verificationType: tEnum("verification_type", [
    ...
    // Verification status
    status: tEnum("verification_status", [
